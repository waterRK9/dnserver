...
def send_query_zone(query, zone,method=""):
    """
    ...
    for nsaddr in nsaddr_list:
        ...
        response = send_query(msg, nsaddr, query, newid=True,method=method)
        if response:
            rc = response.rcode()
...
def resolve_name(query, zone, inPath=True, addResults=None,method=""):
    """
    ...
    while stats.cnt_deleg < MAX_DELEG:
        ...
        response = send_query_zone(query, curr_zone,method)
        if not response:
            return
