...
def send_query_tls(msg, nsaddr, query, timeout=TIMEOUT):
    """Send query over TCP"""
    res = None
    stats.update_query(query, tcp=True)
    try:
        res = dns.query.tls(msg, nsaddr.addr, timeout=timeout)
    except dns.exception.Timeout:
        print("WARN: TLS query timeout for {}".format(nsaddr.addr))
    return res
def send_query_http(msg, nsaddr, query, timeout=TIMEOUT):
    res = None
    stats.update_query(query, tcp=True)
    try:
        with httpx.Client() as client:
            res = dns.query.https(msg, nsaddr.addr, timeout=timeout, session=client)
    except dns.exception.Timeout:
        print("WARN: TLS query timeout for {}".format(nsaddr.addr))
    return res
...
def send_query(msg, nsaddr, query, timeout=TIMEOUT, retries=RETRIES,
    ...
    if method=="tls":
        return send_query_tls(msg, nsaddr, query, timeout=timeout)
    if method=="http":
        return send_query_http(msg, nsaddr, query, timeout=timeout)
